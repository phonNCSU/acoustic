...
plot.vowels <- function(m, param1, param2, group.by='phone', meas=NULL, mf=NULL, limits='global', size_param=FALSE, 
    ...
    for (i in 1:length(levels(afsub[,group.by]))){
        ...
        # print(p)
        afsub_p = subset(afsub, afsub[,group.by]==p)
        if (do.ellipses & from_means){
            # print ('will plot ellipse')
            # print(afsub_p)
            ellipse_x = afsub_p[,paste0(param1,'mean')]
            ellipse_y = afsub_p[,paste0(param2,'mean')]
            # shape_matrix = matrix(c(var(data_x),
            #                         cov(data_x,data_y),
            #                         cov(data_y,data_x),
            #                         var(data_y)),nrow=2)
            shape_matrix = matrix(c(afsub_p[,paste0('var',param1)],
                                    afsub_p[,paste0('cov',param1,param2)],
                                    afsub_p[,paste0('cov',param2,param1)],
                                    afsub_p[,paste0('var',param2)]),nrow=2)
            # print(shape_matrix)
            # print(c(ellipse_x, ellipse_y))
            ellipse(center=c(ellipse_x, ellipse_y), 
                    shape=shape_matrix,
                    radius=1.55, 
                    center.pch=F, col=colors[i], fill=fill)
     ...
     #   lwd=2, fill=FALSE, fill.alpha=0.3, grid=TRUE, id=FALSE, ...) 
        ...
        }else if (length(subset(afsub, afsub[,group.by]==p)[,1])>2 & do.ellipses){
            data_x = c(afsub_p[,param1])
            data_y = c(afsub_p[,param2])
            dataEllipse(data_x, data_y, 
...
optimize_within_speaker = function(my_data, phone_col='phone', best_bw, best_by_ph, mcols = c('F1_50','F2_50','log_B1_50','log_B2_50')){
    ...
    for (i in 1:length(unique(my_data$speaker))){
        ...
        print ('##################################################################')
        print(paste(sp,i,'of',length(unique(my_data$speaker))))
        print ('##################################################################')
...
bark_all_formants <- function(data, formants=1:3){
    for (f in formants){
